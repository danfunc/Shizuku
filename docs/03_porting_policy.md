...
- **D25. 他 ISA でのクロック基準の評価** (2026-08-20 ユーザー依頼: RISC-V (ESP32-C
  ...
  - **結論**: **クロックを契約の単位にするのは 4 つとも正しい**。違うのは
    ...
    割り込みの回数だけ**
  - ★★**この評価で見つかった穴 (未解決)**: サイクルカウンタは**コアが止まって
    いる間は数えない** (WFI / WFE / HLT)。x86 の `CPU_CLK_UNHALTED` は名前どおり、
    ARMv8-M の SysTick もプロセッサクロックを数えるので WFI で止まる (RP2350 で
    実際に止まる)。つまり**借り手が眠ると予算を消費せず、取り上げも起きない** —
    貸し手へ CPU が戻らない。µs 基準のときは壁時計と突き合わせていたので部分的に
    覆えていた。対処は「クロック予算 + 壁時計の非常口」の 2 本立てだが、これは
    貸し借りの契約の意味を変える話 (「N クロックの仕事」なのか「N クロックの仕事、
    ただし壁時計で T まで」なのか) なので、**ユーザーの判断を仰ぐ**
