...
- **D23. オーバークロックは今は入れない (レバーだけ記録する)** (2026-08-20
  ユーザー示唆:「速度が足りなさそうなら、リファレンス実装にオーバークロックの
  方法が載ってる」)。
  - 実測で見ると**今の律速は CPU クロックではない**:
    - 揺らぎ (late win) は budget にそのまま追従する (budget 2000µs → 2.1〜2.6ms、
      100µs → 0.2〜0.3ms)。これはスケジューリング方針の数字で、クロックの数字ではない
    - LED 書き込み 1.7〜3.9ms は CYW43 の SPI 転送。★参照実装のオーバークロックは
      `CYW43_PIO_CLOCK_DIV_INT = ceil(clk_sys / 75MHz)` として **SPI を 75MHz に
      据え置く**設計なので、clk_sys を 2 倍にしても**この時間は縮まらない**
  - 効くのは別のところ: 機構の下限 (〜250µs)、XIP のフラッシュ読み出し速度
    (= flash FS の読み)、フラッシュ書き込み中の停止時間
  - 入れないと決めた理由: 参照実装が記録している失敗が「落ちる」ではなく
    **静かに壊れる**類だから (CYW43 の evt が 68→1 になっても ble_poll は前進するので
    動いて見える / フラッシュは速すぎると落ちずに読み違える)。効かないと分かっている
    レバーのために、その種の危険を今持ち込む理由がない
  - 引くときの作法 (参照実装 CMakeLists §オーバークロック より。**固定分周を直書き
    しない**が全体の教訓): clk_sys 300MHz / vreg 1.20V を既定とし、
    CYW43 PIO 分周 = ceil(clk_sys/75MHz)、フラッシュ QMI = 目標 100MHz から導出、
    RXDELAY は段数でなく **ps** で宣言 (5000ps)、clk_peri は PLL_SYS から
    150MHz へ分周 (`set_sys_clock_khz` は放っておくと 48MHz へ落とす)。
    上げる前に XIP スイープを回すこと
