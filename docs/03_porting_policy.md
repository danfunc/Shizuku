...
- **D30. flash を書く間は他コアを止める (2026-08-21 ユーザー指示:「flash_safe_execute
  ...
  - **★実測 (2026-08-21): 41 バイトのファイル 1 つを書くのに、両方のコアが
    72,441 µs = 72 ms 止まる**。スケジューリングの揺らぎ (1〜2 ms) を 30 倍以上
    上回る。**flash の書き込みは、系の中で最も長く全体を止める操作**であって、
    周期スレッドの隣で気軽に呼ぶものではない。読みは XIP なのでゼロ
    (D21 の「アドレスを返す」がここでも効いている)
