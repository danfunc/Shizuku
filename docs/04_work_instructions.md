...
### Phase 5 進捗 (2026-08-20 実施・**実機確認済み**)

段階を分けて上った (いきなり実オブジェクトを非特権化しない — 参照実装はそれで
HW-FAIL している。HANDOFF §0.7):

**段階 A: MPU (W^X + PRIVDEFENA) を張る。全オブジェクトは特権のまま。**
- region0 = XIP 全窓 RO+X / region1 = ヒープ先頭 (`__end__`) 〜 SRAM 終端 RW+XN
- PRIVDEFENA=1 → **region の外 (静的データ・ペリフェラル) は特権のみ**。これが
  単一アドレス空間における「カーネル空間」の実体 (DESIGN §11.1)
- 機構 (MPU レジスタ) は arch、どこに何を張るか (SoC の地図) は board に分けた
- 実機: 既存の 28 項目・負荷試験・揺らぎとも変化なし = 特権側は無傷 ✓

**段階 B: 非特権オブジェクトを 1 つだけ走らせる。**
- プローブは**グローバルも標準ライブラリもカーネルの簿記も触らない**。それらは
  region の外 = 特権のみなので、触れば落ちる (§11.2.2)
- ★**対象自身に `MRS CONTROL` を読ませて持ち帰らせる**。参照実装は実際には特権の
  まま走っていた計測を「非特権で動いた」と読み、そこから積み上げた観測を全部
  無効にしている (§11.2.0)。自己申告なしにこの主張はできない
- 同じコードを 2 つのオブジェクトとして登録し、片方だけ非特権を宣言する対照実験
- 実機: `control unpriv=15 priv=14` (bit0 = nPRIV が 1 / 0)、呼び出し元は特権の
  まま復帰 ✓、selftest 32/0 ✓。**非特権で走り、syscall を撃って戻ってこられる**

**段階 C: 違反しても系は止まらない (2026-08-20 実機確認済み)**
- フォールト入口を**普通の例外と同じ退避/復帰経路**に乗せた。これで「落ちたスレッドを
  止めて別のスレッドへ切り替えて復帰する」が特別扱い無しにできる
- スレッドモードで落ちた → **そのスレッドだけを止める**。借り手として走っていたなら
  貸し手へ返し (期限切れと同じ形)、そうでなければ composition で教えられた復帰先へ渡す。
  誰に渡すかは方針なのでカーネルは選ばない
- ハンドラモードで落ちた場合だけは止めても直らないので panic (カーネル自身の破れ)
- 拒否のテスト: 非特権オブジェクトに region の外 (SIO の読み出し専用レジスタ) を
  触らせる。**別スレッドで**走らせるのが要点 — 落ちるのはスレッド単位なので、
  呼び出しで試すと自分ごと止まってテストの続きが走れない
- 実機: `[FAULT] thread 3 stopped: pc=1000173a addr=d0000000 cfsr=00000082 (系は継続)`
  — pc は addr2line で `trespass` に一致、addr は触ろうとした先そのもの、cfsr は
  MemManage のデータアクセス違反。selftest 35/0、負荷も点滅も継続
- ★診断で 2 回嘘をついた: (i) 引数スロットを流用して pc を読み、r12 が出ていた
  (ii) 違反の記録を消してから触った先を読み、無効な値が出ていた。**診断は間違えても
  静かに嘘を吐く**ので、専用の読み出し口を用意し、消す前に読む順序を守ること

**次の段 (未実施)**:
1. **オブジェクトごとの arena** (§11.2.2)。状態がヒープに載れば非特権を既定にできる。
   今は opt-in (`OBJECT_UNPRIVILEGED` を宣言した相手だけ)
3. per-object の region 切り替え (§11.3)。CALL の protection に region set を載せる
4. ペリフェラルオブジェクトは特権のまま (`spi_init` は RESETS/CLOCKS を触る)。
   **そこが特権を引き受けることで上のドライバを非特権にできる**、が分担 (D18)
