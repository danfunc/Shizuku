...
**次の段 (未実施)**:
1. **拒否のテスト** (§16「許可のテストだけでは無意味」)。非特権オブジェクトが
   静的データやペリフェラルへ触ったら落ちること、かつ**系が生き残ること**を見る。
   そのためにはフォールトから復帰できる必要がある — 今のフォールトハンドラは
   報告して止まるだけなので、「そのスレッドだけを終わらせて他は走り続ける」形へ
   (§11.2.4 の「違反しても系は止まらない」)
2. **オブジェクトごとの arena** (§11.2.2)。状態がヒープに載れば非特権を既定にできる。
