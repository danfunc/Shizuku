...
enum struct method : uintptr_t {
  ...
  RESET = 8,
  // a1 = UART 経路の入力ストリーム番号 (入力枠 1)。
  // ★★枠を第 2 引数で渡す設計にしない。**CALL_METHOD は引数を 1 つしか
  //   運ばない** (a1=オブジェクト, a2=メソッド番号, a3=引数)。2 つ目を渡した
  //   つもりで黙って捨てられ、UART が配線されないまま「BLE 枠が既に埋まって
  //   いる」と弾かれる、という分かりにくい壊れ方を実際にした (2026-09-02)。
  //   枠ごとにメソッドを分ければ、符号化を間違えようがない。
  SET_INPUT_STREAM_UART = 9,
};
