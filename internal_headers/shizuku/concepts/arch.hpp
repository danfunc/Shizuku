...
// - set_activation_info : 活性化のたびにカーネルが打つ 4 語 (svc 番号 / 呼び出し元
//                       cookie / 呼び出し元スレッド / 今のネスト数) を、呼び先が
//                       受け取れる場所 (callee-saved レジスタ等) へ置く
// - return_stub       : 呼び先が普通に return したときの戻り口アドレス。RETURN
//                       プリミティブを 1 段ぶん発行する ISA 側のコード
...
concept arch_requires =
    requires(typename ARCH::context_t &context,
      ...
      { ARCH::set_activation_info(context, address, address, address, address) };
